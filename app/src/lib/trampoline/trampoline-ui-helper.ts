...
class TrampolineUIHelper {
  ...
  public async getLoginForRepositoryPath(path: string): Promise<string | null> {
    ...
    if (repo) {
      // Ugly hack to avoid infinite loading in Windows (https://github.com/matbrgz/blackfin/issues/119)
      // Additionally, don't await this call, as it could fail and we don't want to show an error to the user in that case (https://github.com/matbrgz/blackfin/issues/146)
